...
后续再次运行时会复用已有计划，重新应用并验证优化参数，而不会重复创建新的电源计划。已有 `LIME_YOUR_PC` 计划的说明文字也会自动同步到当前版本，并改用中文描述，例如 `LIME_YOUR_PC v0.6.1 · 游戏性能电源计划`。
...
v0.6.0 起加入轻量级的软件侧响应测试，用来观察 Windows **定时唤醒 / 线程调度**的响应速度与稳定性。v0.6.1 进一步整理了测试流程：手动测试可以直接复用为一键优化的“优化前”基准；当前 LIME 配置已经完全符合目标时，会跳过没有意义的重复 A/B；如果已有 LIME 计划但当前未启用，则会先测试当前电源计划，再切换至既存 LIME 计划完成前后对比。
...
点击 **“应用 LIME 优化”** 时，程序会根据当前状态决定是否需要自动 A/B：如果你刚刚做过“测试当前状态”，会直接复用该结果；如果当前 LIME 电源计划已经完全符合目标，则跳过没有意义的重复 A/B。需要比较时，程序会记录切换前后的结果并给出结论。

也可以完全不执行一键优化，单独点击 **“测试当前状态”** 获取当前机器的基础结果。需要跨重启比较 HVCI、VBS 等设置时，再使用 **“保存对比基准”** 保存结果，重启后重新测试即可比较。

> 这个模块测量的是 Windows 软件侧调度响应，并不是鼠标点击到显示器像素变化的端到端输入延迟。端到端延迟需要高速相机、光电传感器或 NVIDIA LDAT 一类外部测量设备才能严谨获得。

为了减少误导，LIME 会把很小的前后变化视为正常波动，并显示“无明显变化”；如果两次测试的 CPU 占用或后台进程环境差异较大，也会提示建议复测。
...
System Tweaks
System Response Benchmark / A-B Comparison
```
